...
if __name__ == '__main__':
    ...
    fig.suptitle('Análise do Pêndulo Amortecido Forçado')
    fig.set_size_inches(10, 10)
    axs[0][0].plot(tt1[::],xn1[::],'k',linewidth=.7,label = '$\\gamma = 0.2$')
    axs[0][0].set(xlabel='tempo [s]',ylabel='$\\theta$[rad]')
    axs[0][0].grid()
    axs[0][0].legend()
    axs[0][1].plot(tt2,xn2,'k',linewidth=.7,label='$\\gamma = 0.9$')
    axs[0][1].set(xlabel='t[s]',ylabel='$\\theta$[rad]')
    axs[0][1].legend(loc='lower right')
    axs[1][0].plot(tt3,xn3,'k',linewidth=.7,label='$\\gamma = 1.06$')
    axs[1][0].set(xlabel='t[s]',ylabel='$\\theta$[rad]')
    axs[1][0].legend(loc='lower right')
    axs[1][1].plot(tt4,xn4,'k',linewidth=.7,label='$\\gamma = 1.073$')
    axs[1][1].set(xlabel='t[s]',ylabel='$\\theta$[rad]')
    axs[1][1].legend(loc='upper right')
    #axs[1].set_ylim([-10,10])
    plt.savefig(os.path.join('results', name, 'pendulo_amortecido_forcado.png'))