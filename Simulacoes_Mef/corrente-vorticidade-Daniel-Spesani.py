...
Path(os.path.join(name,'resultados_cilindro_v5_implicito')).mkdir(parents=True, exist_ok=True)
...
for n in range(nIter):
	...
	surf3 = ax[1,0].tricontourf(triang,Z3,50,cmap=matplotlib.cm.jet, extent=(X.min(),
	               X.max(), Y.min(), Y.max()),vmin=0,vmax=2.2)
...
# plt.yticks(laby)
	...
	if(theta == 1):
		subname = 'resultados_cilindro_v5_implicito'
	if(theta == 1/2):
