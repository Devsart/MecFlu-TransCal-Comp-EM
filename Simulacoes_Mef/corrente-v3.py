...
def solveCorrente(theta = 1,dt=0.5,lim_e=1e-5):
    ...
    verts2 = xy[cc]
    fig, ax = plt.subplots()
    surf = ax.tricontourf(X,Y,Z1,50,cmap=matplotlib.cm.jet, extent=(X.min(),
                       X.max(), Y.min(), Y.max()),vmin=0,vmax=1)
    fig.set_size_inches(9, 4)
    ax=plt.gca()
    pc = matplotlib.collections.PolyCollection(verts,edgecolors=('None',),
                                                  facecolor='None',
                                                  linewidths=(0.7,))
